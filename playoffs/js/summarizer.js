...
export class Summarizer {
	constructor(year, teamRepo) {
		this.year = year;
		this.teams = teamRepo.getAllTeams();
	}
	...
	summarizeRound(pickResults) {
		...
		const pointsDict = Object.fromEntries(
			Object.entries(summaries).map(([person, summary]) => [person, summary.points]),
		);
...
function calculateRankMapFromDict(d) {
	...
	return Object.fromEntries(Object.entries(d).map(([person, points]) => [person, excelRank(allPoints, points)]));
}
