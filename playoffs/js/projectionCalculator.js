...
export class ProjectionCalculator {
		...
		for (let games = 4; games <= 7; games++) {
			...
			for (const team of scfTeams) {
				...
				for (const person in points) {
					const roundPick = roundFourPicks[person]?.['O'];
					if (roundPick) {
	...
	calculateIsPossible(team, games) {
		const scfSeries = this.seriesRepo.getScfSeries();
		if (scfSeries.isOver()) {
			if (scfSeries.totalGames() !== games) {
				return false;
			}
			return scfSeries.isTopSeedWinner() ? team === scfSeries.topSeed : team === scfSeries.bottomSeed;
		}
